...
function! s:Project(filename) " <<<
  ...
  if exists("g:proj_running")
    ...
    if strlen(a:filename) == 0
      if filereadable(".vimprojects")
        let filename ='.vimprojects'      " Default local project filename
      else
        let filename ='~/.vimprojects'      " Default user project filename
      endif
    else
  ...
  if !exists('g:proj_flags')
    if has("win32") || has("mac")
      let g:proj_flags='imsthk'             " Project default flags for windows/mac
    else
      let g:proj_flags='imstbhk'            " Project default flags for everything else
    endif
  endif
  ...
  " RecursivelyConstructDirectives(lineno) <<<
  ...
  " ConstructInfo(home, c_d, scriptin, scriptout, flags, filter) <<<
  ...
  " OpenEntry2(line, infoline, precmd, editcmd) <<<
  "   Get the filename under the cursor, and open a window with it.
  ...
  " VimDirListing(filter, padding, separator, filevariable, filecount, dirvariable, dircount) <<<
  ...
  " GenerateEntry(recursive, name, absolute_dir, dir, c_d, filter_directive, filter, foldlev, sort) <<<
  ...
  " DoEntryFromDir(line, name, absolute_dir, dir, c_d, filter_directive, filter, foldlev, sort) <<<
  ...
  " CreateEntriesFromDir(recursive) <<<
  ...
  " RefreshEntriesFromDir(recursive) <<<
  ...
  function! s:RefreshEntriesFromDir(recursive)
    ...
    if just_a_fold == 0
      ...
      if !isdirectory(glob(home))
        call confirm('"'.home.'" is not a valid directory 1.', "&OK", 1)
      else
  ...
  " SetupScriptAutoCommand(bufcmd, script) <<<
  ...
  " GrepAll(recurse, lineno, pattern) <<<
  ...
  function! s:GetScriptin(info, home)
    ...
    if strlen(scriptin) == strlen(a:info) | let scriptin=''
    else  | if !s:IsAbsolutePath(scriptin) | let scriptin=a:home.'/'.scriptin | endif | endif
    return scriptin
  endfunction " >>>

  " GetScriptout(info, home) <<<
  function! s:GetScriptout(info, home)
    ...
    if strlen(scriptout) == strlen(a:info) | let scriptout=''
    else | if !s:IsAbsolutePath(scriptout) | let scriptout=a:home.'/'.scriptout | endif | endif
    return scriptout
  endfunction " >>>
  ...
  " HighlightFile <<<
  " Highlight the current opened file
  function! s:HighlightFile()
    if g:proj_last_buffer == bufnr('%')
      return
    endif
    if bufwinnr(g:proj_running) != winnr()
      ...
      if line > 0
        if Project_GetFname(line) == path
          call s:HighlightLine(line)
        else
          normal G$
          let flags = "w"
          let found = 0
          while !found && search(file, flags) > 0
            let line = line('.')
            if Project_GetFname(line) == path
              call s:HighlightLine(line)
              let found = 1
  ...
  " ResizeProjectWindowIfNeeded() <<<
  function! s:ResizeProjectWindowIfNeeded()
    if winwidth(winnr()) != g:proj_window_width
      exec b:proj_resize_command
    endif
  endfunction " >>>

  " RelocateProjectWindowIfNeeded() <<<
  function! s:RelocateProjectWindowIfNeeded()
  ...
  " EnsureProjectSizeAndPosition <<<
  function! s:EnsureProjectSizeAndPosition()
